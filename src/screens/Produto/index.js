...
export default function ProdutoScreen() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const response = await endpoints.produtos.listar();
  ...
  async function deletarProduto(id) {
    ...
    Alert.alert('Confirmacao', 'Deseja realmente excluir este produto?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await endpoints.produtos.deletar(id);
            await loadData();
          ...
          <View style={styles.card}>
            ...
            <View style={styles.cardInner}>
              <View style={styles.cardContent}>
                ...
                <Text style={styles.title}>Categoria: {getCategoriaNome(item) ?? 'Nao informada'}</Text>
              </View>
              <View style={styles.actions}>
                <TouchableOpacity style={[styles.iconButton, { backgroundColor: '#E3F2FD' }]} onPress={() => editarProduto(item)}>
                  <Text style={styles.iconText}>Editar</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.iconButton, { backgroundColor: '#FFEBEE' }]} onPress={() => deletarProduto(getId(item))}>
